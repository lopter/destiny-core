{
  ...
  outputs = inputs@{ flake-parts, self, ... }:
    flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { config, self', inputs', pkgs, system, setupRustToolchain, ... }: {
        ...
        _module.args.setupRustToolchain = { nightly, withWasm32 }:
        let
          fenixPkgs = inputs'.fenix.packages;
          hostRustToolchain = if nightly == false
            then (fenixPkgs.fromManifestFile inputs.rust-manifest)
            else (fenixPkgs.fromManifestFile inputs.rust-manifest-nightly);
          ...
          wasmRustToolchain = if nightly == false
            then (wasmTarget.fromManifestFile inputs.rust-manifest)
            else (wasmTarget.fromManifestFile inputs.rust-manifest-nightly);
        in
          fenixPkgs.combine (
            hostRustComponents
            ++ pkgs.lib.optionals withWasm32 [ wasmRustToolchain.rust-std ]
          );
        _module.args.setupCraneLib = { nightly, withWasm32 }:
        let
        ...
        devenv.shells.default = {
          ...
          packages = [
            (setupRustToolchain { nightly = false; withWasm32 = false; })
          ] ++ (with pkgs; [
            cfssl
            gnused
            shellcheck
