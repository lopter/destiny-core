...
let
  ...
  mapAttrsRecursiveToList = set:
  let
    makePair = lineage: key:
    let
      path = lineage ++ [key];
    ...
    in
      if builtins.isAttrs value
        then map (makePair path) (builtins.attrNames value)
        else if builtins.stringLength varPrefix > 0
          then "${varPrefix}_${name}=${toString value}"
